...
//  vtkSmartPointer<vtkSMSession> session = vtkSmartPointer<vtkSMSession>::New();
  vtkSMSession* session = vtkSMSession::New((vtkPVSessionCore*)NULL);
  vtkIdType id = vtkSMSession::ConnectToSelf();

 // vtkSMSession* session = new vtkSMSession(true, vtkPVSessionCore::New());
...
//  test.partitioner->SetGhostCellOverlap(test.ghostOverlap);
  ...
  if (test.doRender) {
    ...
    vtkSmartPointer<vtkPVRenderView> view = vtkSmartPointer<vtkPVRenderView>::New();
    vtkSmartPointer<vtkDepthSortRepresentation> repr = vtkSmartPointer<vtkDepthSortRepresentation>::New();
    //
    view->AddRepresentation(repr);
    repr->SetInputConnection(processId->GetOutputPort(0));
    //
    /*
    //
